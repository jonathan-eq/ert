...
def test_queue_options_are_propagated_from_config_to_sbatch(monkeypatch):
    ...
    args.remove("--parsable")  # caused a lot of problems with parsing
    ...
    assert parsed_options.get("--ntasks") == str(expected_num_cpu)
    assert (
        parsed_options.get("--mem")
        == f"{_parse_realization_memory_str(expected_realization_memory) // 1024**2}M"
    )
    assert parsed_options.get("--nodelist") == expected_include_hosts
    assert parsed_options.get("--exclude") == expected_exclude_hosts
    assert parsed_options.get("--time") == _seconds_to_slurm_time_format(
        expected_max_runtime
    )
    assert parsed_options.get("--partition") == expected_partition
    assert parsed_options.get("--account") == expected_project_code
